...
void downloadBootstrap() {
    ...
    if (IsVerium())
        pathBootstrapZip = GetDataDir() / "bootstrap_VRM.zip";

    if (IsVericoin())
        downloadFile(BOOTSTRAP_VRC_URL, pathBootstrapZip);
    else
        downloadFile(BOOTSTRAP_VRM_URL, pathBootstrapZip);
...
void downloadVersionFile() {
    ...
    if (IsVerium())
        pathVersionFile = GetDataDir() / "VERSION_VRM.json";

    if (IsVericoin())
        downloadFile(VERSIONFILE_VRC_URL, pathVersionFile);
    else
        downloadFile(VERSIONFILE_VRM_URL, pathVersionFile);
...
void downloadClient(std::string fileName) {
    ...
    std::string clientFileUrl = CLIENT_VRC_URL + fileName;

    if (IsVerium())
        clientFileUrl = CLIENT_VRM_URL + fileName;
